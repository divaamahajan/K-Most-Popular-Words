...
In this approach, we read the entire data file in a single thread and stores the count of each word in a hashmap. It also reads stop words from a file and removes them from the word counts. The frequency of the words is then sorted in reverse, and the top K words are returned. This approach has a time complexity of O(nlogn) for sorting, where n is the number of words in the dataset. However, it has a space complexity of O(n) as the entire dataset needs to be stored in the hashmap. This approach is suitable for smaller datasets as sorting the entire dataset can be time-consuming for larger datasets. The advantage of using a hashmap is that it provides constant time complexity for accessing and updating the value of a key. The main function prompts the user to input the number of top words to find, gets the file size, reads stop words from a file, and calculates the word counts for the data file. It then prints the top K frequent words and performance statistics, including the file name, start time, file size, and number of top words to find.
